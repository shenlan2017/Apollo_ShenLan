...
if [ $# -lt 2 ]; then
  echo "Usage: build_lane_map.sh [records folder] [map name]"
  exit 1
fi
...
touch map.txt
cat supplement/map_header.txt >> map.txt
cat base_map.txt >> map.txt
mv map.txt base_map.txt
...
-m /apollo/modules/map/data/$2/base_map.txt
