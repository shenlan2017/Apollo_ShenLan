...
DEFINE_string(pcd_folders, "", "provide the pcd folders");
DEFINE_string(pose_files, "", "provide pose files");
...
class OfflineLidarObstaclePerception {
 public:
  ...
  bool setup() {
    ...
    lidar_segmentation_.reset(new LidarObstacleDetection);
 ...
 protected:
  ...
  LidarObstacleDetectionInitOptions segment_init_options_;
  LidarObstacleDetectionOptions segment_options_;
  LidarObstacleTrackingInitOptions tracking_init_options_;
  LidarObstacleTrackingOptions tracking_options_;
  std::unique_ptr<LidarObstacleDetection> lidar_segmentation_;
