...
#         """
#         #  接受前左上的imu信息
#         rfu =  np.array([[0,1,0],[-1,0,0],[0,0,1]])
#         imu_v = np.array([[data.linear_acceleration.x],[data.linear_acceleration.y],[data.linear_acceleration.z]])
#         g = np.array([[0],[0],[9.81]])
#         #  坐标转换右前上
#         time.sleep(0.002)
#         odom =  self.odometry
#         R = np.dot(rfu,np.transpose(odom))
#         imu_v = imu_v - np.dot(R,g)
#         # 利用odometry信息来成生corrected_imu
#         acc_y = float (imu_v[0])
#         if abs(acc_y)<0.0001:
#             acc_y = 0

#         self.acceleration = acc_y
...
#         # 信号量保护
#         r = Rotation.from_quat([data.localization.orientation.qx, data.localization.orientation.qy, data.localization.orientation.qz,data.localization.orientation.qw])
#         self.odometry = r.as_matrix()
...
#                                                 imu_pb2.Imu,
#                                                 data_collector.callback_imu)

#     node.create_reader('/apollo/sensor/gnss/odometry',
#                                             gps_pb2.Gps,
#                                             data_collector.callback_odometry)
