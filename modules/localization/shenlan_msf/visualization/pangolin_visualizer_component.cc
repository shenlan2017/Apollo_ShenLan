...
bool PangolinVisualizerComponent::InitIO() {
  ...
  std::function<void(const std::shared_ptr<LocalizationEstimate> &)>
      gnss_local_call =
          std::bind(&PangolinVisualizerComponent::OnGNSSLocalization, this,
                    std::placeholders::_1);
...
bool PangolinVisualizerComponent::Proc(
  ...
  // TODO: fix this bug
  double timestamp = cyber::Time(msg->measurement_time()).ToSecond() - 0.21;
  LidarMsgTransfer(msg, &lidar_frame);

  CloudData::CloudTypePtr temp_cloud(new CloudData::CloudType());
  Eigen::Matrix4f T_imu_lidar_ = velodyne_extrinsic.matrix().cast<float>();
