 ...
 * @LastEditTime: 2022-09-21 22:16:52
 */
...
OdoPreIntegrator::OdoPreIntegrator(const YAML::Node& node) {
  ...
  B_.block<3, 3>(INDEX_THETA, INDEX_M_W_PREV) = B_.block<3, 3>(
      INDEX_THETA, INDEX_M_W_CURR) = 0.50 * Eigen::Matrix3d::Identity();
}
...
bool OdoPreIntegrator::Update(const VelocityData& velocity_data) {
  if (odo_data_buff_.front().time_ < velocity_data.time_) {
    // set buffer:
...
void OdoPreIntegrator::ResetState(const VelocityData& init_velocity_data) {
  // reset time:
  time_ = init_velocity_data.time_;
...
void OdoPreIntegrator::UpdateState() {
  ...
  // get time delta:
  T = curr_odo_data.time_ - prev_odo_data.time_;

  // get measurements:
  const Eigen::Vector3d prev_w(prev_odo_data.angular_velocity_.x,
                               prev_odo_data.angular_velocity_.y,
                               prev_odo_data.angular_velocity_.z);
  const Eigen::Vector3d curr_w(curr_odo_data.angular_velocity_.x,
                               curr_odo_data.angular_velocity_.y,
                               curr_odo_data.angular_velocity_.z);

  const Eigen::Vector3d prev_v(prev_odo_data.linear_velocity_.x,
                               prev_odo_data.linear_velocity_.y,
                               prev_odo_data.linear_velocity_.z);
  const Eigen::Vector3d curr_v(curr_odo_data.linear_velocity_.x,
                               curr_odo_data.linear_velocity_.y,
                               curr_odo_data.linear_velocity_.z);
  ...
  F_.block<3, 3>(INDEX_ALPHA, INDEX_THETA) =
      -0.50 * (prev_R_v_hat + curr_R_v_hat * dR_inv);
  // F22:
