...
class SlidingWindowComponent final : public apollo::cyber::TimerComponent {
 public:
  ...
  void OnPointCloud(const std::shared_ptr<CloudData>& message);
  ...
  void OnSynchImu(const std::shared_ptr<IMUData>& imu_synced_msg);
 ...
 private:
  ...
  std::shared_ptr<SlidingWindow> sliding_window_ptr_ = nullptr;
  ...
  std::deque<IMUData, Eigen::aligned_allocator<IMUData>> imu_synced_list_;
...
CYBER_REGISTER_COMPONENT(SlidingWindowComponent);
